  ...
  module CloudPowers
    ...
    module Creatable
      ...
      def self.build(name:, **config)
        new_resource = self.new(name, config)
        new_resource = yield new_resource if block_given?
        new_resource
      end
      ...
      def self.create!(name:, **config)
        new_resource = self.build(name, opts)
        new_resource.instance_attr_accessor config
        new_resource.attr_map! config
        new_resource.save!
        new_resource = yield new_resource if block_given?
        new_resource
      end
      ...
      #
      # Returns
      # +Object+ - this instance of the class that extended this module, mapped
      # to an appropriate cloud resource
      def save!
        create_resource
        @saved = true
        self
