require 'spec_helper'

describe 'AwsResources' do
  include Smash::CloudPowers::AwsResources
  include Smash::CloudPowers::Zenv

  before(:all) do
    Dotenv.load("#{project_root}/.test.env")
  end

  it 'should be able to create an EC2 object' do
    expect(ec2).to be_kind_of Aws::EC2::Client
  end

  it 'should be able to get an image from a given name' do
    expect(image('*')).to be_kind_of Aws::EC2::Types::Image
  end

  it 'should be able to create a Kinesis object' do
    expect(kinesis).to be_kind_of Aws::Kinesis::Client
  end

  it 'should be able to create a S3 object' do
    expect(s3).to be_kind_of Aws::S3::Client
  end

  it 'should be able to create a SNS object' do
    expect(sns).to be_kind_of Aws::SNS::Client
  end

  it 'should be able to create a SQS object' do
    expect(sqs).to be_kind_of Aws::SQS::Client
  end
end
